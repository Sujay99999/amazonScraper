...
const speakersScraperUrl = async (url) => {
  try {
    console.log(url);
...
const speakerObjScraper = async (arr) => {
  try {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      fs.writeFileSync(
        `./../speakers/speaker-${i + 1}.json`,
        JSON.stringify(returnedObj)
      );
      console.log("data collected for speaker ", i + 1);
    }
    // return laptopObjArr;
  } catch (err) {
    console.log(err);
...
const puppeteerScrape = async (url, browser) => {
  try {
    ...
    speakerObj.originalPrice = await newPage.evaluate(() => {
    ...
    // if (!speakerObj.originalPrice) return "";

    // required
    speakerObj.finalPrice = await newPage.evaluate(() => {
      if (
    ...
    if (!speakerObj.finalPrice) return "";

    speakerObj.savingPrice = await newPage.evaluate(() => {
    ...
    // if (!speakerObj.savingPrice) return "";

    // required
    speakerObj.fullName = await newPage.evaluate(() => {
    ...
    if (!speakerObj.fullName) return "";

    // required
    speakerObj.description = await newPage.evaluate(() => {
    ...
    if (!speakerObj.description) return "";

    //create a obj for each technicalDetail
    //{ detail:'', value:""}
    // required
    speakerObj.technicalDetails = await newPage.$$eval(
    ...
    if (!speakerObj.technicalDetails) return "";

    //required
    speakerObj.additionalDetails = await newPage.$$eval(
    ...
    if (!speakerObj.additionalDetails) return "";

    // we will have a deafult rating
    speakerObj.totalRating = await newPage.evaluate(() => {
    ...
    // if (!speakerObj.totalReviews) return "";

    speakerObj.reviewArr = await newPage.$$eval(
      ".a-section.review",
      (reviewArr) => {
    ...
    speakerObj.imageArr = await newPage.$$eval(
      ".image .a-dynamic-image",
      (imageArr) => {
        return imageArr.map((el) => el.src);
      }
    );
    if (!speakerObj.imageArr) return "";
...
const main = async () => {
  // let speakerUrlArr = [];
  // speakerUrlArr = speakerUrlArr.concat(
  //   await speakersScraperUrl(
  //     "https://www.amazon.in/s?k=speakers&page=1&qid=1620011266&ref=sr_pg_1"
  //   )
  // );
  // speakerUrlArr = speakerUrlArr.concat(
  //   await speakersScraperUrl(
  //     "https://www.amazon.in/s?k=speakers&page=2&qid=1620011266&ref=sr_pg_2"
  //   )
  // );
  // speakerUrlArr = speakerUrlArr.concat(
  //   await speakersScraperUrl(
  //     "https://www.amazon.in/s?k=speakers&page=3&qid=1620011266&ref=sr_pg_3"
  //   )
  // );
  // speakerUrlArr = speakerUrlArr.concat(
  //   await speakersScraperUrl(
  //     "https://www.amazon.in/s?k=speakers&page=4&qid=1620011266&ref=sr_pg_4"
  //   )
  // );
  // speakerUrlArr = speakerUrlArr.concat(
  //   await speakersScraperUrl(
  //     "https://www.amazon.in/s?k=speakers&page=5&qid=1620011266&ref=sr_pg_5"
  //   )
  // );
  // console.log("final lenght", speakerUrlArr.length);
  // // saving the arr to the json doc
  // fs.writeFileSync(
  //   "./../urlArr/speakerUrlArr.json",
  //   JSON.stringify(speakerUrlArr)
  // );
  // // now create the objects
  // //   let laptopArr = JSON.parse(fs.readFileSync("./laptopUrlArr.json", "utf8"));

  let speakerArr = JSON.parse(
    fs.readFileSync("./../urlArr/speakerUrlArr.json", "utf8")
  );
  console.log(speakerArr.length);
  // speakerArr = speakerArr.slice(0, 1);
  await speakerObjScraper(speakerArr);
};
